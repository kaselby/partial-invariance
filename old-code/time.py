...
n=8
N=300
sizes = torch.linspace(3,7.5,7).exp().round().int()
...
models = {
    ...
    'equi:': MultiSetTransformer(n, 8, 16, 1, equi=True, nn_attn=False).cuda(),
    'nn_attn-equi': MultiSetTransformer(n, 8, 16, 1, equi=True, nn_attn=True).cuda()
}
baseline = kl_knn
