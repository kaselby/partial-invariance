...
do
    ...
    #sbatch scripts/train.sh "${run_name}_cross-only/${i}" $target $data -1 0 $dim $latent_size $hidden_size $lr $clip "cross-only" $basedir 0 $num_blocks $ss1 $ss2 $merge $warmup_steps $steps $dl $rezero
