...
do
    ...
    #sbatch scripts/train.sh "${run_name}_sum-merge/${i}" $target $data -1 0 $dim $(( dim*latent_size )) $(( dim*hidden_size)) $lr $clip "csab" $basedir 0 $num_blocks $ss1 $ss2 "sum" $warmup_steps $steps $dl $residual $scale_out $vardim $batch_size
    ...
    sbatch scripts/train.sh "${run_name}_union/${i}" $target $data -1 0 $dim $(( dim*latent_size )) $(( dim*hidden_size )) $lr $clip "union" $basedir 0 $num_blocks $ss1 $ss2 $merge $warmup_steps $steps $dl $residual $scale_out $vardim $batch_size
